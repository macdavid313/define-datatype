...
(define define-datatype:pretty-print
  (lambda (x)
    (if (debug?)
        (begin
          (write x)
...
(define define-datatype:test3
  '(begin
     (define-datatype define-datatype:test:alist define-datatype:test:alist?
       ...
       (define-datatype:test:apair (head always?) (tail define-datatype:test:blist?)))
...
(test-begin "Functionality")

(define test-apis
  (list define-datatype:test0
...
(for-each (lambda (t)
            (display (eval t (interaction-environment)))
            (newline)
            (test-assert #t))
          test-apis)
...
(for-each (lambda (t)
            (test-error #t (eval t (interaction-environment))))
          test-errors)

(test-end "Errors")

;; mw added dynamic-wind around rebinding of eopl:error-stop
;; (define error-stop "error-stop is purposely undefined")

;; (define define-datatype:tester
;;   (lambda (example)
;;     (display "------------------------------")
;;     (newline)
;;     (define-datatype:pretty-print example)
;;     (display "-->")
;;     (newline)
