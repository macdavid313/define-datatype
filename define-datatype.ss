...
(library (define-datatype)
  (export define-datatype isa cases list-of? always?)
  (import (chezscheme))
  ...
  (define define-datatype:report-error errorf)
  ...
  (define define-datatype:type-registry '())
  (define define-datatype:variant-registry '())

  (define define-datatype:reset-registries
    (lambda ()
      (set! define-datatype:type-registry '())
      (set! define-datatype:variant-registry '())
      #t))

  (define define-datatype:is-a-type?
    (lambda (type-name)
      (memq type-name define-datatype:type-registry)))

  (define define-datatype:datatype-checker&registry-updater
    (letrec ((set?
              (lambda (s)
                (if (null? s) #t
  ...
  (define define-datatype:case-checker
    (let ((remq-or-false
           (lambda (sym ls)
